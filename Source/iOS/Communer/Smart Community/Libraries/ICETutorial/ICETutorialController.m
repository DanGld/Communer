...
@property (nonatomic, strong, readonly) UIView *pageBackgroundView;
...
- (instancetype)initWithPages:(NSArray *)pages {
    ...
    if (self) {
        ...
        _pageBackgroundView = [[UIView alloc] init];
    }
    return self;
}
...
- (void)setupView {
    ...
    //page BackgroundView
    [self.pageBackgroundView setFrame:CGRectMake(0, self.view.frame.size.height - PAGE_BACKGROUND_HEIGHT, [self numberOfPages] * self.view.frame.size.width, PAGE_BACKGROUND_HEIGHT)];
    [self.pageBackgroundView setBackgroundColor:[UIColor colorWithRed:40.f / 255.f green:40.f / 255.f blue:40.f / 255.f alpha:1.0f]];
    ...
    [self.pageControl setCurrentPageIndicatorTintColor:[UIColor colorWithRed:44.f / 255.f green:195.f / 255.f blue:239.f / 255.f alpha:1.0f]];
...
- (void)addAllConstraints {
    ...
    [constraints addObject:[NSString stringWithFormat:@"V:[_pageControl(==%d)]-%d-|", PAGE_CONTROL_SIZE_HEIGHT, (int)(PAGE_BACKGROUND_HEIGHT - PAGE_CONTROL_SIZE_HEIGHT) / 2]];
    [constraints addObject:[NSString stringWithFormat:@"H:|-%d-[_pageControl(==%d)]", (int)(self.view.frame.size.width - PAGE_CONTROL_SIZE_WIDTH) / 2, PAGE_CONTROL_SIZE_WIDTH]];
...
- (void)setOverlayTexts {
    ...
    for (ICETutorialPage *page in self.pages) {
        ...
        if ([[[page subTitle] text] length]) {
            [self overlaySubLabelWithStyle:[page subTitle]
...
- (void)overlaySubLabelWithStyle:(ICETutorialLabelStyle *)style
                  commonStyle:(ICETutorialLabelStyle *)commonStyle
                        index:(NSUInteger)index {
    // SubTitles.
    UILabel *overlayLabel = [[UILabel alloc] initWithFrame:CGRectMake((index * self.view.frame.size.width) + TUTORIAL_SUBTITLE_OFFSET,
                                                                      self.view.frame.size.height - [commonStyle offset],
                                                                      self.view.frame.size.width  - TUTORIAL_SUBTITLE_OFFSET * 2,
                                                                      TUTORIAL_SUBLABEL_HEIGHT)];
    [overlayLabel setNumberOfLines:[commonStyle linesNumber]];
    [overlayLabel setBackgroundColor:[UIColor clearColor]];
    [overlayLabel setTextAlignment:NSTextAlignmentCenter];
